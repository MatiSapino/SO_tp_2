...
void drawChar(uint64_t hexColor, char character) {
    ...
    for (int i = 0; i < 32; i++) {
        if (i % 2 == 0 && i != 0) {
            y += font_size;  // Salto a la siguiente fila de píxeles
        ...
        font[i + (start * 32)] & (char)0x01 ? put_square(a, y, font_size, hexColor) : 0;
        
        a += font_size;  // Avanza a la siguiente posición horizontal
        
        uint8_t aux = 0x02;
        for (int j = 0; j < 8; j++) {
            // Comprueba cada bit de la fuente y dibuja un píxel si está activo
            ((uint8_t)font[i + (start * 32)] & (uint8_t)aux) >> j ? put_square(a, y, font_size, hexColor) : 0;
            a += font_size;  // Avanza a la siguiente posición horizontal
...
void moveOneLineUp() {
    ...
    char* src = dst + VBE_mode_info->pitch * (int)font_size * 16; // Puntero a la línea de origen
    uint64_t numBytes = VBE_mode_info->pitch * (VBE_mode_info->height - font_size * 16); // Cantidad de bytes a copiar

    memcpy(dst, src, numBytes); // Copia los bytes desde la línea de origen a la línea de destino
    // memset((void*)(uintptr_t)(VBE_mode_info->framebuffer + VBE_mode_info->pitch * (VBE_mode_info->height - font_size * 16)), 0, VBE_mode_info->pitch * font_size * 16); // Rellena con ceros la parte de la línea de destino copiada
    drawRectangle(bg_color, 0, VBE_mode_info->height - font_size*16, 1024, font_size*16 );
    cursorY -= (font_size * 16); // Actualiza la posición del cursor en el eje Y
}
