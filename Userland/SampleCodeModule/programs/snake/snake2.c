...
void check_player_crash()
{
    ...
    {
        if (posHead1.x == 0 || posHead1.x == mapSize || posHead1.y == 0 || posHead1.y == mapSize)
        { // check p1 out of bounds
            kill_player_1();
            singleplayer_gameover();
        }
        for (int i = 1; i < player1_size; i++)
        {
            if (posHead1.x == posBody1[i].x && posHead1.y == posBody1[i].y)
            { // check if p1 crashed against itself
                kill_player_1();
                singleplayer_gameover();
            }
        }
    ...
    {
        if (posHead1.x == 0 || posHead1.x == mapSize || posHead1.y == 0 || posHead1.y == mapSize)
        { // check if p1 crashed against edge
            if (player1_alive == 1)
            {
                kill_player_1();
            }
        }
        if (posHead2.x == 0 || posHead2.x == mapSize || posHead2.y == 0 || posHead2.y == mapSize)
        { // check if p2 crashed against edge
            if (player2_alive == 1)
            {
                kill_player_2();
            }
        ...
        for (int i = 0; i < player2_size; i++)
        {
            if (posHead1.x == posBody2[i].x && posHead1.y == posBody2[i].y)
            { // check if p1 crashed against p2
                if (player1_alive == 1)
                {
                    kill_player_1();
                    player1_alive = 0;
            ...
            if (i != 0 && posHead2.x == posBody2[i].x && posHead2.y == posBody2[i].y)
            { // check if p2 crashed against itself
                if (player2_alive == 1)
                {
                    kill_player_2();
                    player2_alive = 0;
        ...
        for (int i = 0; i < player1_size; i++)
        {
            if (posHead2.x == posBody1[i].x && posHead2.y == posBody1[i].y)
            { // check if p2 crashed against p1
                if (player2_alive == 1)
                {
                    kill_player_2();
                    player2_alive = 0;
